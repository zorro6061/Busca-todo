...
test.describe('ApertureZen Smoke Tests (Rev 100)', () => {
  ...
  test('Test 2: Login Flow (Bypass Rafael)', async ({ page }) => {
    ...
    const galleryLink = page.locator('nav a:has-text("Galería"), .mobile-nav-item:has-text("Buscados")');
  ...
  test('Test 3: Blueprint Generation (Solid Geometry)', async ({ page }) => {
    ...
    await page.waitForSelector('.variant-item', { state: 'visible', timeout: 10000 });
