...
class Plano(db.Model):
    ...
    homografia_json = db.Column(
        db.Text, nullable=True
    )  # Matriz 3x3 serializada para proyección
    ubicaciones = db.relationship("Ubicacion", backref="plano", lazy=True)
    muebles = db.relationship(
        "Mueble", backref="plano", lazy=True, cascade="all, delete-orphan"
    )
...
class Ubicacion(db.Model):
    ...
    objetos = db.relationship(
        "Objeto", backref="ubicacion", lazy=True, cascade="all, delete-orphan"
    )
    ...
    mueble_texto = db.Column(
        db.String(100), nullable=True
    )  # Ej: Estante, Mesa de noche
    punto_especifico = db.Column(
        db.String(150), nullable=True
    )  # Ej: Cajón derecho, Parte alta
...
class Objeto(db.Model):
    ...
    ubicacion_id = db.Column(
        db.Integer, db.ForeignKey("ubicaciones.id"), nullable=True, index=True
    )
    zona_id = db.Column(
        db.Integer, db.ForeignKey("zonas.id"), nullable=True, index=True
    )
    ...
    posicion_relativa = db.Column(
        db.String(50), nullable=True
    )  # izquierda, derecha, etc.
    contenedor = db.Column(
        db.String(100), nullable=True
    )  # cajón superior, estante medio

    # Campo para Inteligencia Semántica (Fase 19)
    tags_semanticos = db.Column(
        db.Text, nullable=True
    )  # Sinónimos, usos y contextos (IA)
...
class Mueble(db.Model):
    ...
    nombre = db.Column(
        db.String(100), nullable=True
    )  # Nombre personalizado (ex. Estante A1)
